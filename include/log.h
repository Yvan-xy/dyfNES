...
#define LOG(level) \
if (level > dyfNES::Log::get().getLevel()) ; \
else dyfNES::Log::get().getStream() << '[' << __FILENAME__ << ":" << std::dec << __LINE__ << "] "

#define LOG_CPU \
if (dyfNES::CpuTrace != dyfNES::Log::get().getLevel()) ; \
else dyfNES::Log::get().getCpuTraceStream()
